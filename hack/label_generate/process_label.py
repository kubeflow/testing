...
class LabelColorMapping(object):
    color_mapping_general = {
        ...
        'platform': '2515fc',
        'problems': 'fc2515',
        'priority': {'p0': 'db1203', 'p1': 'db03cc', 'p2': 'fc9915'},
        'status': {'approved': '03db12', 'backlog': 'bc9090', 'done': 'fca42e',
                   'icebox': '808080', 'in progress': 'ffa500', 'lgtm': '2efca4'},
        'default': '00daff'
    }
...
def csv_to_yml():
    input_csv = os.path.dirname(os.path.abspath(__file__)) + "/kubeflow_label_standard.csv"
    ...
    with open(input_csv) as raw_csv:
        ...
        for line in raw_csv:
            ...
            label_ins = {'name': label_name.lower(), 'color': LabelColorMapping.get_color(label_name.lower())}
...
def get_curr_labels():
    ...
    repo_labels = []
    for repo in [str(repo_info['name']) for repo_info in requests.get('https://api.github.com/orgs/kubeflow/repos').json()]:

        for label in requests.get('https://api.github.com/repos/kubeflow/%s/labels' % repo).json():
