...
def get_s3_dir(bucket):
    ...
    if not build:
        logging.warning(
            "BUILD_ID not set; trying BUILD_NUMBER; BUILD_NUMBER is deprecated"
        )
        build = os.getenv("BUILD_NUMBER")

    if job_type == "presubmit":
        output = (
            "s3://{bucket}/pr-logs/pull/{owner}_{repo}/" "{pull_number}/{job}/{build}"
        ).format(
            bucket=bucket,
            owner=repo_owner,
            repo=repo_name,
        ...
        output = ("s3://{bucket}/logs/{owner}_{repo}/" "{job}/{build}").format(
            bucket=bucket, owner=repo_owner, repo=repo_name, job=job_name, build=build
        )
    else:
        # Its a periodic job
        output = ("s3://{bucket}/logs/{job}/{build}").format(
            bucket=bucket, job=job_name, build=build
        )
...
def check_no_errors_s3(s3_client, artifacts_dir):
    ...
    junit_objects = s3_client.list_objects(
        Bucket=bucket, Prefix=os.path.join(prefix, "junit")
    )
...
def main(unparsed_args=None):  # pylint: disable=too-many-locals
    ...
    parser_copy.add_argument(
        ...
        type=str,
        help=(
            "Optional if supplied add this suffix to the names of all artifact "
    ...
    parser_link = subparsers.add_parser(
        ...
        help="Create a symlink pointing at PR output dir in S3; null "
    ...
    test_log = os.path.join(
        os.path.join(args.artifacts_dir, "artifacts"),
        "logs",
